...
import type { ClaudeBackgroundSnapshot, ClaudeBackgroundTasks } from './background.ts'
import {
...
interface TaskOutputProjection {
  readonly retrievalStatus: 'success' | 'timeout' | 'not_ready'
  readonly id: string
...
function taskType(kind: string): string {
  if (kind === 'bash') return 'local_bash'
  if (kind === 'subagent') return 'local_agent'
  return kind
}

function formatTaskOutput(task: TaskOutputProjection): string {
  const parts = [
  ...
  if (task.exitCode !== undefined) parts.push(`<exit_code>${task.exitCode}</exit_code>`)
  if (task.output.trim().length > 0) parts.push(`<output>\n${task.output.trimEnd()}\n</output>`)
  if (task.error !== undefined) parts.push(`<error>${task.error}</error>`)
  return parts.join('\n\n')
}

function nativeJob(value: unknown): NativeJobProjection | undefined {
  if (!isRecord(value) || !isRecord(value.job)) return undefined
  const job = value.job
  if (
...
function nativeJobOutput(value: unknown): string {
  return isRecord(value) && typeof value.text === 'string' ? value.text : ''
}

function exitCode(detail: string | undefined): number | undefined {
...
function backgroundProjection(
  task: ClaudeBackgroundSnapshot,
  retrievalStatus: TaskOutputProjection['retrievalStatus'],
...
export class ClaudeTaskStore {
  private readonly lists = new Map<string, TaskListState>()

  /** Share background-agent lifecycle state with Agent and SendMessage. */
  constructor(private readonly background: ClaudeBackgroundTasks) {}

  private list(exec: Parameters<ClaudeToolBody>[1]): TaskListState {
  ...
  bodies(ctx: Context): Record<string, ClaudeToolBody> {
    return {
      ...
      TaskGet: async (args, exec) => {
        ...
        if (task.blockedBy.size > 0) lines.push(`Blocked by: ${[...task.blockedBy].map(value => `#${value}`).join(', ')}`)
        if (task.blocks.size > 0) lines.push(`Blocks: ${[...task.blocks].map(value => `#${value}`).join(', ')}`)
        return lines.join('\n')
      },
      ...
      TaskUpdate: async (args, exec) => {
        ...
        for (const blocked of stringArray(args.addBlocks)) task.blocks.add(blocked)
        if (stringArray(args.addBlocks).length > 0) updated.push('blocks')
        for (const blocker of stringArray(args.addBlockedBy)) task.blockedBy.add(blocker)
        if (stringArray(args.addBlockedBy).length > 0) updated.push('blockedBy')
      ...
      TaskOutput: async (args, exec) => {
        ...
        if (background !== undefined) {
          ...
          if (!active) return formatTaskOutput(backgroundProjection(background, 'success'))
          if (!block) return formatTaskOutput(backgroundProjection(background, 'not_ready'))
          const settled = await this.background.wait(id, timeout, exec.signal)
          const current = this.background.get(id)
          if (current === undefined) throw new Error(`No task found with ID: ${id}`)
        ...
        if (job === undefined) throw new Error('job_output returned an invalid task record')
      ...
      TaskStop: async (args, exec) => {
        ...
        if (id === undefined) throw new Error('Either task_id or shell_id must be provided')
        const background = this.background.get(id)
        if (background !== undefined) {
        ...
        if (result.isError) throw new Error(result.error.message)
        const job = nativeJob(result.value)
        if (job === undefined) throw new Error('job_kill returned an invalid task record')
