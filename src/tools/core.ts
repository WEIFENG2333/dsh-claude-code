...
import { resolveClaudeWorkflow } from './workflow.ts'
...
function contentValue(content: unknown): JsonValue {
  return { content: structuredClone(content) } as JsonValue
}
...
function nestedError(result: Extract<Awaited<ReturnType<typeof dispatchNativeResult>>, { isError: true }>): never {
...
function formatReadValue(value: JsonValue, offset: number): string {
  if (!isRecord(value) || !Array.isArray(value.lines)) return resultText(value)
  const totalLines = typeof value.totalLines === 'number' ? value.totalLines : 0
  const lines = value.lines.filter((line): line is Record<string, JsonValue> => isRecord(line))
  if (lines.length === 0) {
    return totalLines === 0
...
async function readPdf(
  ...
  if (pages !== undefined && range === null) throw new Error(`Invalid pages parameter: "${pages}". Use formats like "1-5", "3", or "10-20". Pages are 1-indexed.`)
  const first = range?.[1]
  ...
  if (!isRecord(value) || value.kind !== 'foreground' || !isRecord(value.stdout)) return resultText(value)
...
export function coreToolBodies(ctx: Context, options: CoreToolOptions): Record<string, ClaudeToolBody> {
  return {
    Read: async (args, exec) => {
      ...
      if (IMAGE_EXTENSIONS.has(extension)) {
        ...
        if (result.isError) nestedError(result)
        return contentValue(result.content)
      }
      if (extension === '.pdf') return readPdf(ctx, filePath, optionalStringArg(args, 'pages'), exec)
    ...
    Bash: async (args, exec) => {
      ...
      if (!isRecord(value)) return resultText(value)
      ...
      if (value.aborted === true) parts.push('<error>Command was aborted before completion</error>')
    ...
    WebFetch: async (args, exec) => answerWebFetch(
      ctx,
      stringArg(args, 'url'),
