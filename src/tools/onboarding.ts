...
function validCode(code: string): string {
  if (!/^[A-Za-z0-9_-]{1,64}$/u.test(code)) throw new Error('Invalid onboarding guide short_code')
  return code
}
...
export class LocalOnboardingGuides {
  ...
  private guidePath(code: string): string {
    return resolve(this.root, `${validCode(code)}.md`)
  }

  private async latestCode(): Promise<string | undefined> {
    const candidates: Array<{ code: string; modified: number }> = []
    for (const entry of await readdir(this.root, { withFileTypes: true }).catch(() => [])) {
      if (!entry.isFile() || !entry.name.endsWith('.md')) continue
      const code = entry.name.slice(0, -3)
      if (!/^[A-Za-z0-9_-]{1,64}$/u.test(code)) continue
  ...
  async run(args: Readonly<Record<string, unknown>>, exec: Parameters<ClaudeToolBody>[1]): Promise<string> {
    const mode = stringArg(args, 'mode')
    const requestedCode = optionalStringArg(args, 'short_code')
    await mkdir(this.root, { recursive: true })
    if (mode === 'delete') {
      if (requestedCode === undefined) throw new Error('delete requires short_code')
      const code = validCode(requestedCode)
      if (await existingFile(this.guidePath(code)) === undefined) throw new Error(`No onboarding guide with short_code '${code}'`)
    ...
    if (mode === 'create') {
      if (source === undefined) throw new Error(`No ONBOARDING.md found at ${sourcePath}`)
    ...
    if (mode === 'update') {
      if (requestedCode === undefined) throw new Error('update requires short_code')
      if (source === undefined) throw new Error(`No ONBOARDING.md found at ${sourcePath}`)
      const code = validCode(requestedCode)
      if (await existingFile(this.guidePath(code)) === undefined) throw new Error(`No onboarding guide with short_code '${code}'`)
    ...
    if (mode !== 'check') throw new Error(`Unsupported onboarding guide mode: ${mode}`)
