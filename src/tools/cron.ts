...
function expandField(field: string, range: FieldRange): number[] | undefined {
  ...
  for (const part of field.split(',')) {
    ...
    if (span !== null) {
      ...
      if (lower > upper || step < 1 || lower < range.min || upper > maximum) return undefined
      for (let value = lower; value <= upper; value += step) output.add(dayOfWeek && value === 7 ? 0 : value)
      continue
    }

    if (/^\d+$/u.test(part)) {
      let value = Number.parseInt(part, 10)
      if (range.min === 0 && range.max === 6 && value === 7) value = 0
...
export function parseCron(expression: string): CronFields | undefined {
  ...
  if (expanded.some(field => field === undefined)) return undefined
  return {
    minute: expanded[0] ?? [],
    hour: expanded[1] ?? [],
...
export function nextCronRun(expression: string, from: Date): Date | undefined {
...
function localTime(minute: number, hour: number): string {
...
export function humanCron(expression: string): string {
  ...
  if (!/^\d+$/u.test(minute ?? '') || !/^\d+$/u.test(hour ?? '')) return expression
  const value = Number.parseInt(minute ?? '', 10)
  ...
  if (dayOfMonth === '*' && month === '*' && /^\d$/u.test(dayOfWeek ?? '')) {
    ...
    if (day !== undefined) return `Every ${day} at ${localTime(value, hourValue)}`
  }
