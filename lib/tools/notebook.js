...
import { dispatchNative, optionalStringArg, stringArg, } from "./runtime.js";
function isRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
function readText(value) {
    if (!isRecord(value) || !Array.isArray(value.lines))
        throw new Error('Read did not return a text file');
    return value.lines
...
function sourceLike(existing, source) {
...
export function notebookToolBodies(ctx, workspace) {
    return {
        NotebookEdit: async (args, exec) => {
            ...
            if (editMode === 'delete') {
                ...
                if (optionalStringArg(args, 'cell_type') !== undefined)
