...
import { dispatchNative, optionalBooleanArg, optionalStringArg, stringArg, } from "./runtime.js";
...
function stdout(value) {
    if (!isRecord(value) || !isRecord(value.stdout))
        return '';
    return String(value.stdout.text ?? '').trim();
}
async function git(ctx, cwd, command, description, exec, suffix) {
    ...
    bodies(ctx) {
        return {
            EnterWorktree: async (args, exec) => {
                ...
                await git(ctx, root, `worktree add -b ${shellQuote(branch)} ${shellQuote(path)} ${shellQuote(originalHead)}`, `Create worktree ${rawName}`, exec, 'worktree-add');
            ...
            ExitWorktree: async (args, exec) => {
                ...
                const discarded = [];
                if (commits > 0)
                    discarded.push(`${commits} ${commits === 1 ? 'commit' : 'commits'}`);
                if (changed > 0)
                    discarded.push(`${changed} uncommitted ${changed === 1 ? 'file' : 'files'}`);
