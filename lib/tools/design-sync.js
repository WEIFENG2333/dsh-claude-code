...
export class DesignSyncStore {
    ...
    assetsPath(projectId) {
        return resolve(this.projectDir(projectId), '.dsh-assets.json');
    }
    validationPath(projectId) {
        return resolve(this.projectDir(projectId), '.dsh-validation.json');
    }
    async metadata(projectId) {
        const parsed = JSON.parse(await readFile(this.metadataPath(projectId), 'utf8'));
        if (!isRecord(parsed) || typeof parsed.name !== 'string')
            throw new Error(`Design project ${projectId} is invalid`);
        return parsed;
    }
    ...
    body() {
        return async (args, exec) => {
            ...
            switch (method) {
                ...
                case 'get_project':
                    return await this.metadata(stringArg(args, 'projectId'));
                case 'list_files': {
                    const projectId = stringArg(args, 'projectId');
                    await this.metadata(projectId);
                    return await walk(this.projectDir(projectId));
                }
                case 'get_file': {
                    ...
                    if (info.size > 256 * 1024)
                        throw new Error('Remote design file exceeds the 256 KiB limit');
                ...
                case 'finalize_plan': {
                    ...
                    const plan = {
                        projectId,
                        writes: Array.isArray(args.writes) ? args.writes.filter((value) => typeof value === 'string') : [],
                        deletes: Array.isArray(args.deletes) ? args.deletes.filter((value) => typeof value === 'string') : [],
                ...
                case 'write_files': {
                    ...
                    if (plan === undefined)
                        throw new Error('write_files requires a valid finalized planId');
                    const projectId = stringArg(args, 'projectId');
                    if (projectId !== plan.projectId)
                        throw new Error('planId belongs to another project');
                    const files = Array.isArray(args.files) ? args.files.filter(isRecord) : [];
                    for (const file of files) {
                        const projectPath = stringArg(file, 'path');
                        if (!allowed(projectPath, plan.writes))
                            throw new Error(`Path is outside the finalized write plan: ${projectPath}`);
                ...
                case 'delete_files': {
                    ...
                    if (plan === undefined)
                        throw new Error('delete_files requires a valid finalized planId');
                    const projectId = stringArg(args, 'projectId');
                    if (projectId !== plan.projectId)
                        throw new Error('planId belongs to another project');
                    const paths = Array.isArray(args.paths) ? args.paths.filter((value) => typeof value === 'string') : [];
                    for (const path of paths) {
                        if (!allowed(path, plan.deletes))
                            throw new Error(`Path is outside the finalized delete plan: ${path}`);
                ...
                case 'register_assets': {
                    const plan = this.plans.get(stringArg(args, 'planId'));
                    if (plan === undefined)
                        throw new Error('register_assets requires a valid finalized planId');
                    const projectId = stringArg(args, 'projectId');
                    if (projectId !== plan.projectId)
                        throw new Error('planId belongs to another project');
                    ...
                    const registered = new Map((Array.isArray(existing) ? existing : [])
                        .filter(isRecord)
                        .filter(asset => typeof asset.path === 'string')
                        .map(asset => [String(asset.path), asset]));
                    for (const asset of assets) {
                        const path = stringArg(asset, 'path');
                        if (!allowed(path, plan.writes))
                            throw new Error(`Asset is outside the finalized write plan: ${path}`);
                ...
                case 'unregister_assets': {
                    const plan = this.plans.get(stringArg(args, 'planId'));
                    if (plan === undefined)
                        throw new Error('unregister_assets requires a valid finalized planId');
                    const projectId = stringArg(args, 'projectId');
                    if (projectId !== plan.projectId)
                        throw new Error('planId belongs to another project');
                    const paths = Array.isArray(args.paths) ? args.paths.filter((value) => typeof value === 'string') : [];
                ...
                case 'report_validate': {
                    const projectId = stringArg(args, 'projectId');
                    await this.metadata(projectId);
                    const counts = isRecord(args.counts) ? structuredClone(args.counts) : null;
