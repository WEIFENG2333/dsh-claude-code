...
function readInstruction(path, kind, seen) {
    try {
...
function append(files, seen, path, kind) {
...
export function captureClaudeInstructions(cwd = process.cwd(), configDir = process.env.CLAUDE_CONFIG_DIR ?? resolve(homedir(), '.claude')) {
    if (/^(?:1|true|yes)$/iu.test(process.env.CLAUDE_CODE_DISABLE_CLAUDE_MDS ?? ''))
        return undefined;
    const files = [];
    const seen = new Set();
    append(files, seen, '/etc/claude-code/CLAUDE.md', 'Managed');
    for (const path of ruleFiles('/etc/claude-code/rules'))
        append(files, seen, path, 'Managed');
    append(files, seen, join(configDir, 'CLAUDE.md'), 'User');
    for (const path of ruleFiles(join(configDir, 'rules')))
        append(files, seen, path, 'User');
    const directories = [];
    ...
    for (const directory of directories.reverse()) {
        ...
        for (const path of ruleFiles(join(directory, '.claude', 'rules')))
            append(files, seen, path, 'Project');
