...
export function captureRuntimeEnvironment(model = CLAUDE_CODE_BASELINE.defaults.model, cwd = process.cwd(), memoryDirectory = resolve(process.env.CLAUDE_CONFIG_DIR ?? resolve(homedir(), '.claude'), 'memory') + '/') {
...
function hydrateSystemText(text, environment) {
    return text
        ...
        .replace(`\n\n${RUNTIME_TOKENS.gitStatus}`, environment.gitStatus === undefined ? '' : `\n\ngitStatus: ${environment.gitStatus}`);
}
/** Materialize the captured system blocks for one immutable startup snapshot. */
export function materializeCapturedSystem(environment = captureRuntimeEnvironment()) {
...
export function capturedSystemPrompt(model = CLAUDE_CODE_BASELINE.defaults.model) {
...
export function capturedAgentContext() {
    ...
    if (block === undefined)
        throw new Error('generated Claude Code agent context is missing');
    return block.text;
}
/** Render the current-date reminder while preserving every captured byte except the date. */
export function currentDateReminder(now = new Date(), timeZone, instructions) {
    ...
    return CLAUDE_CODE_BASELINE.initialContext.currentDateReminderBlock.text
        .replaceAll(RUNTIME_TOKENS.instructions, instructions === undefined || instructions.length === 0 ? '' : `# claudeMd\n${instructions}\n`)
...
export function assertCapturedToolSurface(tools) {
    if (tools === undefined)
        throw new Error('Claude Code request requires the captured tool surface');
...
function replayContent(message) {
    ...
    if (!isRecord(envelope) || !isRecord(envelope.response))
        return undefined;
...
function wireContent(blocks, role = 'user') {
    return blocks.flatMap(block => {
        switch (block.type) {
            ...
            default:
                throw new Error(`unsupported DSH content block: ${block.type}`);
        }
    });
}
function appendWireMessage(messages, message) {
...
function initialMessages(options, config) {
    const firstUserIndex = options.messages.findIndex(message => message.source.kind === 'user');
    if (firstUserIndex < 0)
        throw new Error('Claude Code request requires a user message');
    const firstUser = options.messages[firstUserIndex];
    if (firstUser === undefined)
        throw new Error('Claude Code request user message disappeared');
    const reminder = {
...
async function configuredDeviceId(config) {
...
function requestSessionId(options, config) {
...
export async function buildClaudeRequest(options, config) {
...
export function buildAuxiliaryRequest(options, config) {
    const sessionId = requestSessionId(options, config);
    return {
        sessionId,
        body: {
